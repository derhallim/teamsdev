...
3.  Update the URL in the browser to load the tab created by the scaffolding
...
4.  Now let's load the tab in Microsoft Teams. In the browser, navigate to
    <https://teams.microsoft.com> and if prompted, sign in with your lab admin
    credentials.

5.  Click on **Use web client instead.**

6.  In the left navigation pane, select the **More added apps** button **(…)**.
    Then select **More apps**.

    ![](media/04643990c34f3038c166c4bab85f7534.png)

7.  Under **Apps** section, click on **Upload a custom app** and then **Upload
    for Contoso**.
...
9.  Refresh your web browser window. Microsoft Teams will display a tile for
    **YouTube Player** app. Click on this tile.

    ![](media/9c1e7868cd8deaef419a1e8a9e3960d3.png)

10.  Here you can see some **TODO** items to address. You'll update the todo
    items later in the exercise\*.\*

   ![](media/01b5ba71d32ef6fc17666ecf1e367a3e.png)

11. Select the **Add** button to install the app. You will be taken to the new
    tab.
...
12.  On your **Command Prompt**, stop the running process by pressing Ctrl+C and
...
7.  This JSON code instructs Microsoft Teams to render a textbox and button.
    When the button is selected, it will submit the card.

8.  Copy and paste this JSON into a new file, **YouTubeSelectorCard.json**, into
    the folder
...
3.  Enter a message to post to the channel. Select some of the text and then
    click on **insert link** option to add a link to the message:

    ![](media/72bba3da08079ad40918716e5f6b3a7a.png)

4.  In **Insert link** panel, in **Address** field, provide the link
...
5.  Replace the **{{APP_ID}}** with the **APPLICATION_ID** value available in
...
7.  Now click **Send** to send this message.

8.  Now, click on the link to see the task module open without having to trigger
    it from the custom tab.

9.  Stop the local web server by selecting Ctrl+C in the console to stop the
    running process.

10.  For **Terminate batch job (Y/N)?**, enter **Y** and press enter key.
...
15.  Select **Settings** from the left-hand navigation. Scroll down to the
    **Microsoft App ID** section. Copy the ID of the bot to a notepad, as you'll
    need it later.

   ![](media/837151c79edcfdb4bcd372d3b327603f.png)

16.  Select **Manage** to navigate to the Azure AD app blade.
...
2.  Locate the properties **MICROSOFT_APP_ID** and
...
5.  Locate the **id** property and change its value to the same ID as the
        Azure AD application created for the bot.

6.  Locate the section "bots": [], in the **manifest.json** file. Replace
...
9.  Create a new sub-folder named **learningTeamsBot**.

10.  In this new sub-folder, create a new file named **learningTeamsBot.ts**.

11.  Add the following code to the **learningTeamsBot.ts** file:
...
14.  Add the following line to the to this file:
